...
def create_corpus():
    with open('all.tsv', 'r') as f, open('articles_fr.tsv', 'w') as out:
        out.write(f.readline())
        for raw_line in f:
            (id_article, titre, chapo, texte, langue, auteur, mots) = raw_line.split('\t')
            if (langue == 'fr'):
                out.write(raw_line)
...
def get_article(n, tsv_file):
    ...
    with open(tsv_file, 'r') as f:
        ...
        for i, line in enumerate(f):
            if i == n:
                (id_article, titre, chapo, texte, langue, auteur, mots, date) = line.split('\t')
                return {'titre': titre, 'texte':   texte, 'date': date, 'auteur': auteur}
        
        raise Error('Article non trouvé')
